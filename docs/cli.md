...
The default is interactive when attached to a terminal. Use `--non-interactive` in an agent or CI
session. `--check` rescans the host and marks filesystem checkpoints complete. `--json` emits a
versioned plan with the same steps, snippets, cautions, and checks so a future modal dialog can
render the exact same flow.
...
the lazy component glob and eager metadata glob identical. Use `error-page` only as an explicit
Markdown migration policy; `.svx` remains executable and fail-fast. Acrolls does not choose the
host adapter or deployment provider.
